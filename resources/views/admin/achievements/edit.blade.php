...
<div class="mb-8">
    ...
    <h3 class="text-2xl font-black text-slate-800 tracking-tight leading-tight">Perbarui Data Prestasi</h3>
    <p class="text-[10px] text-slate-400 font-bold uppercase tracking-widest mt-1">Mengubah informasi untuk {{ $achievement->title }}</p>
</div>

<div class="max-w-5xl" x-data="achievementForm()">
    <form action="{{ route('admin.achievements.update', $achievement) }}" method="POST" enctype="multipart/form-data" @submit="loading = true">
        @csrf
        @method('PUT')
                    ...
                    <div class="space-y-6">
                        ...
                        <div>
                            ...
                            <textarea name="description" rows="8" 
                                      ...
                                      placeholder="Jelaskan detail mengenai kompetisi, penyelenggara, dan siapa yang meraih prestasi ini...">{{ old('description', $achievement->description) }}</textarea>
                        </div>
                    </div>
                </div>
            </div>
