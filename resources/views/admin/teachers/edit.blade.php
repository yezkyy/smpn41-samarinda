...
<div class="mb-8">
    ...
    <h3 class="text-2xl font-black text-slate-800 tracking-tight leading-tight">Perbarui Data Personel</h3>
    <p class="text-[10px] text-slate-400 font-bold uppercase tracking-widest mt-1">Mengubah informasi untuk {{ $teacher->name }}</p>
</div>

<div class="max-w-6xl" x-data="teacherForm()">
    <form action="{{ route('admin.teachers.update', $teacher) }}" method="POST" enctype="multipart/form-data" @submit="loading = true">
        @csrf
        @method('PUT')
                    ...
                    <div class="space-y-6">
                        <div>
                            ...
                            <input type="text" name="name" value="{{ old('name', $teacher->name) }}" 
                                   ...
                                   required placeholder="Nama lengkap beserta gelar">
                            @error('name')
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                ...
                                <input type="text" name="birth_place_date" value="{{ old('birth_place_date', $teacher->birth_place_date) }}" 
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div>
                                ...
                                <input type="text" name="social_media" value="{{ old('social_media', $teacher->social_media) }}" 
                                       class="w-full bg-slate-50 border-slate-100 rounded-2xl shadow-sm focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500 py-4 px-5 border transition-all font-bold text-slate-700 placeholder:text-slate-300" 
                        ...
                        <div>
                            ...
                            <textarea name="address" rows="3" 
                                      class="w-full bg-slate-50 border-slate-100 rounded-2xl shadow-sm focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500 py-4 px-5 border transition-all font-bold text-slate-700 placeholder:text-slate-300">{{ old('address', $teacher->address) }}</textarea>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="space-y-6">
                        ...
                        <div>
                            ...
                            <textarea name="education_history" rows="5" 
                                      class="w-full bg-slate-50 border-slate-100 rounded-2xl shadow-sm focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500 py-4 px-5 border transition-all font-bold text-slate-700 placeholder:text-slate-300"
                                      placeholder="Sebutkan riwayat pendidikan (satu per baris)...">{{ old('education_history', $teacher->education_history) }}</textarea>
                        </div>
                        ...
                        <div>
                            ...
                            <textarea name="employment_history" rows="5" 
                                      class="w-full bg-slate-50 border-slate-100 rounded-2xl shadow-sm focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500 py-4 px-5 border transition-all font-bold text-slate-700 placeholder:text-slate-300"
                                      placeholder="Sebutkan riwayat pekerjaan (satu per baris)...">{{ old('employment_history', $teacher->employment_history) }}</textarea>
                        </div>
                    </div>
                </div>
                ...
                <div class="bg-white rounded-[2rem] shadow-sm border border-slate-100 p-8">
                    ...
                    <div>
                        <textarea name="motivation_message" rows="4" 
                                  ...
                                  placeholder="Tuliskan pesan motivasi atau kutipan favorit...">{{ old('motivation_message', $teacher->motivation_message) }}</textarea>
                    </div>
                </div>
            </div>
            ...
            <div class="lg:col-span-4 space-y-6">
                <div class="bg-white rounded-[2rem] shadow-sm border border-slate-100 p-8 flex flex-col items-center sticky top-8">
                    ...
                    <div class="relative group">
                        <div class="w-40 h-40 rounded-[2.5rem] overflow-hidden border-4 border-slate-50 shadow-inner bg-slate-50 ring-1 ring-slate-100 relative">
                            ...
                            <label for="photo" class="absolute inset-0 bg-indigo-600/60 opacity-0 group-hover:opacity-100 transition-opacity flex flex-col items-center justify-center text-white cursor-pointer backdrop-blur-sm">
                                ...
                                <span class="text-[9px] font-black uppercase tracking-widest">Ubah Foto</span>
                            </label>
                        </div>
                    ...
                    <div class="mt-6 text-center">
                        <p class="text-[9px] text-slate-400 font-bold uppercase tracking-[0.15em] leading-relaxed">Format: JPG, PNG, WEBP<br>Biarkan kosong jika tidak diubah</p>
                    </div>
